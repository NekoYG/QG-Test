...
void GameApp::UpdateScene(float dt)
{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D3) && m_CurrMode != ShowMode::StrangeCrate)
	{
		m_CurrMode = ShowMode::StrangeCrate;
		m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
		auto meshData = Geometry::CreateBox();
		ResetMesh(meshData);
		m_pd3dImmediateContext->VSSetShader(m_pVertexNewShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShader(m_pPixelNewShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShaderResources(0, 2, m_pFlameCrate->GetAddressOf());
	}

	if (m_CurrMode == ShowMode::WoodCrate)
	{
		static float phi1 = 0.0f, theta = 0.0f;
		phi1 += 0.0001f, theta += 0.00015f;
		XMMATRIX W = XMMatrixRotationX(phi1) * XMMatrixRotationY(theta);
	...
	else if (m_CurrMode == ShowMode::StrangeCrate)
	{
		static float phi1 = 0.0f, theta = 0.0f, rotate = 0.0f;
		phi1 += 0.0001f, theta += 0.00015f;
		XMMATRIX W = XMMatrixRotationX(phi1) * XMMatrixRotationY(theta);
		...
		rotate += 0.001f;
		m_VSConstantBuffer.texMat = XMMatrixRotationZ(rotate);
	...
	if (m_CurrMode == ShowMode::WoodCrate)
	{
		//m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
			...
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pice.GetAddressOf());
			m_pd3dImmediateContext->DrawIndexed(6, 6, 0);
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pwater.GetAddressOf());
			...
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFlameCrate[0].GetAddressOf());
...
bool GameApp::InitEffect()
{
	...
	//************************************************
	// NEW创建顶点着色器(3D)
	HR(CreateShaderFromFile(L"HLSL\\New_VS_3D.cso", L"HLSL\\New_VS_3D.hlsl", "New_VS_3D", "vs_5_0", blob.ReleaseAndGetAddressOf()));
	HR(m_pd3dDevice->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pVertexNewShader3D.GetAddressOf()));
	// NEW创建像素着色器(3D)
	HR(CreateShaderFromFile(L"HLSL\\New_PS_3D.cso", L"HLSL\\New_PS_3D.hlsl", "New_PS_3D", "ps_5_0", blob.ReleaseAndGetAddressOf()));
	HR(m_pd3dDevice->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_pPixelNewShader3D.GetAddressOf()));
...
bool GameApp::InitResource()
{
	...
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\brick.dds", nullptr, m_pbrick.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\floor.dds", nullptr, m_pfloor.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\ice.dds", nullptr, m_pice.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\water.dds", nullptr, m_pwater.GetAddressOf()));

	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flare.dds", nullptr, m_pFlameCrate[0].GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"Texture\\flarealpha.dds", nullptr, m_pFlameCrate[1].GetAddressOf()));

	// 初始化纹理数组
	WCHAR strFile[40];
	m_pFireAnims.resize(11);
	for (int i = 1; i <= 11; ++i)
	{
		wsprintf(strFile, L"Texture\\Blackman\\B%03d.png", i);
	...
	D3D11SetDebugObjectName(m_pPixelNewShader3D.Get(), "PixelNewShader3D");
